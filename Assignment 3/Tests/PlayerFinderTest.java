...
class PlayerFinderTest {
	...
	void test1() {
		...
		classes.PlayerFinder testObj = new classes.PlayerFinder();
		points = testObj.findPlayers(photo, 3, 16);
	...
	void test2() {
		...
		classes.PlayerFinder testObj = new classes.PlayerFinder();
		points = testObj.findPlayers(photo, 4, 16);
		for (int i = 0; i < solution.length; i++) {
			assertEquals(points[i], solution[i]);
		}
	...
	void test3() {
		...
		classes.PlayerFinder testObj = new classes.PlayerFinder();
		points = testObj.findPlayers(photo, 8, 9);
		for (int i = 0; i < solution.length; i++) {
			assertEquals(points[i], solution[i]);
		}
	...
	void test4() {
		...
		classes.PlayerFinder testObj = new classes.PlayerFinder();
		points = testObj.findPlayers(photo, 1, 3);
		for (int i = 0; i < solution.length; i++) {
			assertEquals(points[i], solution[i]);
		}
